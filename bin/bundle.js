...
function start(root, model, _ref) {
  ...
  var handleEvent = function handleEvent(action) {
    ...
    state$(update(currentState, action, handleEvent));
  };
...
var assocMany = function assocMany(coll, pairs) {
  return pairs.reduce(function (acc, next) {
    return assoc.apply(undefined, [acc].concat(_toConsumableArray(next)));
  }, coll);
};

var model = exports.model = hashMap('fetch', false, 'loading', false, 'data', [], 'page', 1, 'pageSize', 20);

var init = exports.init = function init() {
  return model;
};
var initAndFetch = exports.initAndFetch = function initAndFetch() {
  return assocMany(model, [['fetch', true]]);
};
...
var update = exports.update = function update(model, action, event) {
  return Action.case({
    InitGet: function InitGet() {
      return assocMany(model, [['fetch', false], ['loading', true]]);
    },
    Get: function Get() {
      return request('GET', apiUrl + '/data').getBody().then(function (d) {
        var res = parse(d);
        var words = res.split('\n');
        return assocMany(model, [['loading', false], ['data', words]]);
      });
    },
...
function init() {
  ...
  window.s = start(root, list.initAndFetch(), list);
